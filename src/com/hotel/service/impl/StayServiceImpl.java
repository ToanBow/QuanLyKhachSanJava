...
public class StayServiceImpl implements IStayService {
    ...
    public boolean checkIn(String roomId, Guest guest, Invoice bookingDetails) {
        try {
            ...
            if (room == null || !"Sẵn sàng".equals(room.getStatus())) return false;

            Guest existingGuest = guestDAO.findByCccd(guest.getCccd());
            if (existingGuest == null) {
                if (!guestDAO.insert(guest)) return false;
            }
            ...
            if (!invoiceDAO.insert(bookingDetails)) return false; 
    ...
    public boolean updateStayInformation(String roomId, Guest updatedGuest) {
        ...
        if (activeInvoice == null) return false;
        ...
        try {
            ...
            try (PreparedStatement psGuest = conn.prepareStatement(sqlUpsertGuest)) {
                ...
                if (updatedGuest.getBirthDate() != null) psGuest.setDate(5, new java.sql.Date(updatedGuest.getBirthDate().getTime()));
                else psGuest.setNull(5, java.sql.Types.DATE);
            ...
            if (!activeInvoice.getGuestCccd().equals(updatedGuest.getCccd())) {
                try (PreparedStatement psInvoice = conn.prepareStatement("UPDATE invoices SET guest_cccd = ? WHERE invoice_id = ?")) {
                    psInvoice.setString(1, updatedGuest.getCccd()); psInvoice.setString(2, activeInvoice.getInvoiceId());
    ...
    public void manageRoomServices(String roomId, Service service, int quantity, boolean isAddition) {
        ...
        String sql = isAddition 
            ? "INSERT INTO service_usage (invoice_id, service_id, quantity, price_at_time) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE quantity = quantity + VALUES(quantity)"
            : "DELETE FROM service_usage WHERE invoice_id = ? AND service_id = ?";

        try (Connection conn = DBConnection.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, activeInvoice.getInvoiceId()); ps.setString(2, service.getServiceId());
            if (isAddition) { ps.setInt(3, quantity); ps.setDouble(4, service.getPrice()); }
            ps.executeUpdate();
    ...
    public Invoice processCheckOut(String roomId) {
        ...
        long totalMinutes = 0;
        if (invoice.getCheckInTime() != null) totalMinutes = Duration.between(invoice.getCheckInTime(), invoice.getCheckOutTime()).toMinutes();
        
        long hours = totalMinutes / 60;
        if (totalMinutes % 60 > 15) hours++;
        if (hours == 0) hours = 1;

        // 1. Tính toán Tiền Phòng cơ bản
        ...
        // 2. Kéo dữ liệu Tiền Dịch Vụ từ CSDL lên
        double serviceTotal = 0;
        try (Connection c = DBConnection.getConnection();
             PreparedStatement ps = c.prepareStatement("SELECT SUM(quantity * price_at_time) as svc FROM service_usage WHERE invoice_id = ?")) {
            ps.setString(1, invoice.getInvoiceId());
            ResultSet rs = ps.executeQuery();
            if (rs.next()) serviceTotal = rs.getDouble("svc");
        } catch (Exception e) { e.printStackTrace(); }

        // 3. TÍNH TOÁN TOÀN DIỆN (CỘNG GỘP DỊCH VỤ VÀ PHỤ THU, RỒI TRỪ CỌC & KHUYẾN MÃI)
        double subTotal = roomCost + serviceTotal + invoice.getEarlySurcharge() + invoice.getLateSurcharge();
        double discountAmt = subTotal * (invoice.getDiscount() / 100.0);
        double finalTotal = subTotal - discountAmt - invoice.getDeposit();
        
        if (finalTotal < 0) finalTotal = 0;
        invoice.setTotalAmount(finalTotal);
        invoice.setStatus("PAID");
        if (invoice.getPaymentMethod() == null) invoice.setPaymentMethod("Tiền mặt");
        ...
        try {
            ...
            try (PreparedStatement ps = conn.prepareStatement("UPDATE rooms SET status = 'Chưa dọn' WHERE room_id = ?")) {
